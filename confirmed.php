...
if (strlen($_SESSION['login']) == 0) {
	...
	<head>
		<title>TourEase - Confirmed Bookings</title>
			...
			<div class="privacy">
				<div class="container">
					<h3 class="wow fadeInDown animated animated text-center" data-wow-delay=".5s"
						style="visibility: visible; animation-delay: 0.5s; animation-name: fadeInDown;">Confirmed Bookings</h3>
					...
					<table class="text-center new-table align-self-center align-items-center" width="100%" border="1">
						...
						<!-- Inside the table loop for confirmed bookings -->
<?php
...
$sql = "SELECT tblbooking.BookingId as bookid, tblbooking.PackageId as pkgid, tbltourpackages.PackageName as packagename, tblbooking.FromDate as fromdate, tblbooking.status as status, tblbooking.RegDate as regdate, tblbooking.CancelledBy as cancelby, tblbooking.UpdationDate as upddate, tblbooking.addsOn as addOn, tblbooking.child as child, tblbooking.adult as adult, tblbooking.senior as senior, tblbooking.total as price
        ...
        WHERE UserEmail=:uemail AND tblbooking.status = 1";
...
if ($query->rowCount() > 0) {
    foreach ($results as $result) {
		...
		<td><?php echo htmlentities($cnt); ?></td>
            ...
            <td>Confirmed</td>
            <!-- Add a form to include the hidden input field for price -->
            <td>
			<?php
            // Check the payment status for the current BookingId
            $bookingId = $result->bookid;
            $paymentStatusQuery = $dbh->prepare("SELECT status FROM payment WHERE BookingId = :bookingId");
            $paymentStatusQuery->bindParam(':bookingId', $bookingId, PDO::PARAM_STR);
            $paymentStatusQuery->execute();
            $paymentStatusResult = $paymentStatusQuery->fetch(PDO::FETCH_ASSOC);

            if ($paymentStatusResult && $paymentStatusResult['status'] == 'Success') {
                // If payment status is success, display View Receipt button
                ?>
                <form method="POST" action="view-receipt.php">
                    <input type="hidden" name="bookingId" value="<?php echo htmlentities($result->bookid); ?>">
                    <button type="submit">View Receipt</button>
                </form>
            <?php } else { ?>
                <!-- If payment status is not success, display Pay Now button -->
                <form method="POST" action="razorpay/Razor-check.php">
                    <input type="hidden" name="bookingId" value="<?php echo htmlentities($result->bookid); ?>">

...
// Construct the SQL query to filter packages by location
$sql = "SELECT * FROM tbltourpackages WHERE PackageLocation = :location";
$query = $dbh->prepare($sql);
$query->bindParam(':location', $selectedLocation, PDO::PARAM_STR);
$query->execute();

// Fetch the packages
$result = $query->fetchAll(PDO::FETCH_OBJ);

// Check if packages were found for the selected location
if (count($result) > 0) {
